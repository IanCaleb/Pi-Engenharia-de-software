<x-app-layout>
    <x-sidebar-nav-user active="buscar-lojas">
        ...
        {{-- Ocupa toda a área disponível após a sidebar, sem max-width nem margens automáticas --}}
        <div class="w-full bg-white px-8 py-8 min-h-screen">
                ...
                <div
                    ...
                    <div class="px-4 py-3">
                        ...
                        <div x-show="cidade !== ''" x-transition class="mt-3 space-y-2">
                            ...
                            <button class="w-full rounded-full border px-4 py-1.5 text-sm transition
            ...
            <section class="mb-8">
                <h2 class="mb-4 text-lg font-semibold text-gray-800">Próximo de você:</h2>

                <div class="space-y-3">
                    @foreach ($lojasProximas as $loja)
                    <div x-data="{ aberto: false }" class="rounded-xl border border-gray-200 bg-gray-50 px-5 py-4 shadow-sm">
                        <div class="flex items-center justify-between gap-3">
                            <div class="flex min-w-0 items-center gap-4">
                                <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white shadow-sm text-base">
                                    🎁
                                </div>
                                <div class="min-w-0">
                                    <p class="truncate font-semibold text-gray-800">{{ $loja['nome'] }}</p>
                                    <p class="truncate text-xs text-gray-500">{{ $loja['cidade'] }} • {{ $loja['status'] }} • {{ $loja['data'] }}</p>
                                </div>
                            </div>
                        ...
                        <div x-show="aberto" x-transition class="ml-[52px] mt-4 space-y-1">
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm font-semibold text-gray-800">
                                <span>{{ $loja['remessa']['titulo'] }}</span>
                                <span class="flex items-center gap-1 text-xs font-normal text-gray-500">
                                    <svg class="h-3.5 w-3.5 shrink-0 text-[#841A1A]" viewBox="0 0 24 24" fill="none">
                            ...
                            <p class="text-xs leading-relaxed text-gray-500">{{ $loja['remessa']['endereco'] }}</p>
                        </div>
                        @endif
                    </div>
                    @endforeach
            ...
            {{-- ── Seção: Últimas Lojas ── --}}
            <section>
                <h2 class="mb-4 text-lg font-semibold text-gray-800">Ultimas Lojas:</h2>
